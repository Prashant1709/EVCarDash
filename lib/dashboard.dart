...
class _DashboardState extends State<Dashboard> {
  Timer? _timer;
  int speedc=10;
  double batteryStatus = 100.0; // Initial battery status
  ...
  void _startTimer(){
    _timer=    Timer.periodic(Duration(milliseconds: delayMilliseconds), (timer) {
      if (simulationDuration <= 0) {
        ...
        final driveData = generateEVDriveData();
        setState(() {
          speedc=int.parse(driveData['speed'].toString());
          charge=int.parse(driveData['batteryStatus'].toString());
          powercon=int.parse(driveData['powerConsumption'].toString());
          dte=int.parse(driveData['DTE'].toString());
        });
        print('Speed: ${driveData['speed']} km/h');
        print('Battery Status: ${driveData['batteryStatus']}%');
        print('Power Consumption: ${driveData['powerConsumption']} kWh/100 km');
        print('Distance Traveled: ${driveData['distanceTraveled']} km');
        print('DTE: ${driveData['DTE']} Km');
        simulationDuration--;
        if(charge==0){
          timer.cancel();
          showDialog(context: context, builder: (BuildContext bs)=>AlertDialog(
            title: Text("Please recharge!"),
          ));
        }
      }
    });
  ...
  int simulationDuration = 100; // Number of iterations for the simulation
  int delayMilliseconds = 500; // Delay between iterations in milliseconds
  bool increasingSpeed=true;
  Map<String, dynamic> generateEVDriveData() {
    // Simulate changes in speed and power consumption
    double powerConsumption = 0.1 + (speed / 20); // kWh per 100 km

    // Simulate battery discharge based on power consumption
    batteryStatus -= powerConsumption;
    // Ensure battery status does not go below 0
    if (batteryStatus < 0) {
      batteryStatus = 0;
    }
    // Update speed linearly
    if (increasingSpeed) {
      speed += 2.0; // Increase speed by 1 km/h
      if (speed >= 120) {
        increasingSpeed = false;
      }
    } else {
      speed -= 7.0; // Decrease speed by 1 km/h
      if (speed <= 0) {
        increasingSpeed = true;
      }
    }
    double timeInterval = 0.5; // Time interval in hours (e.g., 6 minutes)
    distanceTraveled += (speed * timeInterval) / 100; // Convert to km
    double powerCsn = 0.1 + (speed / 20); // kWh per 100 km
    double dte = (batteryStatus / powerCsn); // DTE in km
    ...
    return {
      'speed': speed.toInt(),
      'batteryStatus': batteryStatus.toInt(),
      'powerConsumption': powerConsumption.toInt(),
      'distanceTraveled':distanceTraveled.toInt(),
      'DTE':dte.toInt()
    };
  }
                                  ...
                                  children: [
                                    Column(
                                      children: [
                                        ...
                                        GearAndBattery(
                                          charge: charge,usage: powercon,dte:dte,
                                            constraints: constraints),
                                      ],
                                    ),
                                    Positioned(left: MediaQuery.of(context).size.width*0.445,top: MediaQuery.of(context).size.height*0.35,
                                        child: ElevatedButton(
                                          ...
                                          style: ElevatedButton.styleFrom(
                                            ...
                                            primary: isCarStarted ? Colors.green : Color(0xff6B4339).withOpacity(1),
                                          ),
                                          child: Text(isCarStarted ? 'Stop EV' : 'Drive EV'),
                                        ),),
                                    ...List.generate(
